...
if(isset($_POST['user'])){//判断当前是否存在user值。
    ...
    $result = $conn->query("select * from registry where username='$user'");//根据前端出入的用户名检测数据中是否存在结果。
...
if(isset($_POST['submit'])){//前端点击了用户注册按钮，这里才开始取值，判断前端是否点击了注册按钮。
    ...
    $conn->query("insert registry1 values(null,'$user','$pass','$repass','$tel')");
    //第一项null，表示sid是自动递增，无符号(数据库表里面进行设置)

    //跳转登录页面,前端和后端通信采用绝对路径(完整的路径),其他都采用相对路径
    header('location:http://10.31.155.60/JS2104/taobao/src/login.html');
}
