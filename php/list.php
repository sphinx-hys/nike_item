...
$pagesize = 15; //自定义每页的数据条数,单个页面展示的数据条数 自定义每页的条数

$pagenum = ceil($num / $pagesize); //获取页数，一定选择向上取整。 6页
...
//根据传入的页码，计算起始的偏移值。
// limit 后面有两个参数
// 参1：起始的偏移值的索引，从0开始   60数据(0-59)
// 参2：数据的条数
// select * from shoplist limit 0,10 -> 查询所有的数据 从0索引位置开始(第一个数据) 获取10条
// select * from shoplist limit 10,10 -> 查询所有的数据 从10索引位置开始(第一个数据) 获取10条

// 1.select * from shoplist limit 0 ,10  
// 2.select * from shoplist limit 10,10
// 3.select * from shoplist limit 20,10
// 3.select * from shoplist limit 30,10
// 3.select * from shoplist limit 40,10

$page = ($pagevalue - 1) * $pagesize; //根据limit的第一个参数和页码得到这个等式。

$sql1 = "select * from taobaogoods limit $page,$pagesize";//这里的sql语句跟页码有关，返回当前和页码相对应的数据。
